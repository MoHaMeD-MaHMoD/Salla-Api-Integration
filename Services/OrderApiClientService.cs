...
namespace SallaIntegration.Services
{
        public class OrderApiClientService
    {
            ...
            public OrderApiClientService(HttpClient httpClient, IHttpContextAccessor httpContextAccessor)
            {
        ...
        /*
            public async Task<List<ProductJsonResponse.Datum>> GetProductsAsync()
            {
                ...
                var allProducts = new List<OrderJsonResponse.Datum>();
                int currentPage = 1;
                int totalPages;

                do
                {
                    var response = await _httpClient.GetAsync($"https://api.salla.dev/admin/v2/orders?page={currentPage}");
                    ...
                    var productResponse = JsonConvert.DeserializeObject<OrderJsonResponse.Root>(json);
